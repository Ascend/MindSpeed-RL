...
# Logger instance for GRPO training
logger = Loggers("grpo_train")


@ray.remote
def train(config: Dict[str, Any]) -> None:
    """Execute GRPO training process.

    Initializes Ray actor groups for actor, reference, and reward workers,
    sets up datasets and data loaders, and runs the training loop.

    Args:
        config: Training configuration dictionary (Dict[str, Any]) containing 
            all hyperparameters and settings for actor, reference, reward, RL, 
            generation, and profiler. Passed from main() via Ray remote call.
    """
    (
        actor_config,
        ref_config,
    ...
    if rl_config.rule_reward:
        ...
        for i in range(rule_reward_num_process):
            ...
            rule_reward.initialize.remote(
                ...
                rl_config,
                tokenizer,
                generate_config.trust_remote_code,
                dp_rank=i
            )
...
def main(config: DictConfig) -> None:
    """Hydra main entry point for GRPO training.
