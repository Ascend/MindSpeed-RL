...
def sft_train(args):
    ...
    # build dataset
    train_valid_test_num_samples = get_train_valid_test_num_samples(train_samples=args.train_samples,
        train_iters=args.train_iters, global_batch_size=args.global_batch_size,
        eval_interval=args.eval_interval, eval_iters=args.eval_iters)
    train_dataset, valid_dataset, test_dataset \
        = build_train_valid_test_datasets(
        data_prefix=args.data_path, splits_string=args.split, seq_length=args.seq_length + args.num_nextn_predict_layers,
        train_valid_test_num_samples=train_valid_test_num_samples, dataset_cls=InstructionDataset, tokenizer=tokenizer,
        parallel_state=parallel_state, full_shuffle_instruction_dataset=args.full_shuffle_instruction_dataset,
        no_shuffle=args.no_shuffle, reset_position_ids=args.reset_position_ids, prompt_type=args.prompt_type,
        prompt_type_path=args.prompt_type_path, seed=args.seed)
    ...
    if train_dataset is not None and len(train_dataset) > 0:
        train_dataloader = InstructionDataLoader(dataset=train_dataset, parallel_state=parallel_state, tokenizer=tokenizer,
            ...
            micro_batch_size=args.micro_batch_size, comsumed_samples=args.consumed_train_samples, seed=args.seed)
    ...
    if valid_dataset is not None and len(valid_dataset) > 0:
        ...
        pad_to_multiple_of=args.pad_to_multiple_of if args.variable_seq_lengths else args.seq_length + args.num_nextn_predict_layers,
        variable_seq_lengths=args.variable_seq_lengths, num_nextn_predict_layers=args.num_nextn_predict_layers,
        micro_batch_size=args.micro_batch_size, comsumed_samples=args.consumed_valid_samples, seed=args.seed)
    ...
    if test_dataset is not None and len(test_dataset) > 0:
        test_dataloader = InstructionDataLoader(dataset=test_dataset, parallel_state=parallel_state, tokenizer=tokenizer,
            ...
            pad_to_multiple_of=args.pad_to_multiple_of if args.variable_seq_lengths else args.seq_length + args.num_nextn_predict_layers,
            variable_seq_lengths=args.variable_seq_lengths, num_nextn_predict_layers=args.num_nextn_predict_layers,
            micro_batch_size=args.micro_batch_size, comsumed_samples=0, seed=args.seed)
    ...
    # configure Trainer
    megatron_modules = {'train_func': train, 'parallel_state': parallel_state, 'save_checkpoint_func': save_checkpoint,
        'evaluate_fun': evaluate_and_print_results, "generate_seq_len_fun": generate_actual_seq_len, 'batch_cp_func': get_batch_on_this_cp_rank}

    trainer = SFTTrainer(args=args, model=model, optimizer=optimizer, train_data_iterator=train_data_iterator,
        valid_data_iterator=valid_data_iterator, test_data_iterator_list=test_data_iterator_list,
        scheduler=opt_param_scheduler, process_non_loss_data_func=None, model_config=model_arch_config,
        **megatron_modules)
...
def gpt_model_provider(pre_process, post_process):
    ...
    model = GPTModel(config=config, transformer_layer_spec=transformer_layer_spec, vocab_size=args.padded_vocab_size,
        max_sequence_length=args.max_position_embeddings, pre_process=pre_process, post_process=post_process,
        fp16_lm_cross_entropy=args.fp16_lm_cross_entropy, parallel_output=True, rotary_percent=args.rotary_percent,
        share_embeddings_and_output_weights=not args.untie_embeddings_and_output_weights,
        position_embedding_type=args.position_embedding_type, seq_len_interpolation_factor=args.rotary_seq_len_interpolation_factor)

    return model


def initialize_megatron(extra_args_provider=None, args_defaults={}, ignore_unknown_args=False,
        allow_no_cuda=False, skip_mpu_initialization=False, config=None):
