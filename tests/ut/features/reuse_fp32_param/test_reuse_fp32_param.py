...
def initialize_gpt_model(pre_process=True, post_process=True, seed=0, **config_kwargs):
    ...
    default_config_kwargs = dict(num_layers=8, hidden_size=512, num_attention_heads=32,
...
class TestReuseFP32Param(DistributedTest):
    ...
    def test_reuse_float16_params_optimizer(self, tp_pp, is_deterministic):
        ...
        for _, float16_group in enumerate(optimizer.chained_optimizers[0].float16_groups):
        ...
        for _ in range(10):
            for i_index, float16_group in enumerate(optimizer.chained_optimizers[0].float16_groups):
        ...
        for _, float16_group in enumerate(reuse_optimizer.chained_optimizers[0].float16_groups):
            self.before_offloader_data_list = []
            self.after_onloader_data_list = []
        ...
        for _ in range(10):
            for i_index, float16_group in enumerate(reuse_optimizer.chained_optimizers[0].float16_groups):
                for j_index, p in enumerate(float16_group):
                    self.before_offloader_data_groups[i_index][j_index] = p.data
        ...
        reuse_params = copy.deepcopy(list(itertools.chain(*reuse_optimizer.chained_optimizers[0].float16_groups)))
    ...
    def test_reuse_distributed_optimizer(self, tp_pp, is_deterministic, overlap_grad_reduce, overlap_param_gather):
        ...
        for _, float16_group in enumerate(optimizer.chained_optimizers[0].model_float16_groups):
            self.before_offloader_data_list = []
            self.after_onloader_data_list = []
        ...
        for _ in range(10):
            for i_index, float16_group in enumerate(optimizer.chained_optimizers[0].model_float16_groups):
                for j_index, p in enumerate(float16_group):
                    self.before_offloader_data_groups[i_index][j_index] = p.data
        ...
        for _, float16_group in enumerate(reuse_optimizer.chained_optimizers[0].model_float16_groups):
            self.before_offloader_data_list = []
            self.after_onloader_data_list = []
        ...
        for _ in range(10):
            for i_index, float16_group in enumerate(reuse_optimizer.chained_optimizers[0].model_float16_groups):
                for j_index, p in enumerate(float16_group):
                    self.before_offloader_data_groups[i_index][j_index] = p.data
            ...
            reuse_optimizer.step()
            if overlap_param_gather:
                for model_chunk in reuse_optimizer.model_chunks:
                    model_chunk.start_param_sync(force_sync=True)
                torch.cuda.synchronize()


        reuse_params = copy.deepcopy(list(itertools.chain(*reuse_optimizer.chained_optimizers[0].model_float16_groups)))
