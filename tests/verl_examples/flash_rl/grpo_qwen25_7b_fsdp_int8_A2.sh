...
QWEN_PATH=${QWEN_PATH:-"${RAY_DATA_HOME}/models/Qwen2.5-7B-w8a8"}
...
flashrl profile -m ${MODEL_PATH} -q ${QWEN_PATH} -o ${PROFILE_PATH} --fn int8
flashrl setup -m ${QWEN_PATH} -p ${PROFILE_PATH} --fn int8 -o ${CONFIG_PATH}
