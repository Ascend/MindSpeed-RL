...
# 修改为当前需要跑的用例路径
DEFAULT_SH="./test_grpo_qwen2.5_32b_sglang_A2_8k.sh"
echo "Use $DEFAULT_SH"
...
if [ "$MASTER_ADDR" = "$CURRENT_IP" ]; then CURRENT_IP=$(ifconfig $SOCKET_IFNAME | grep -Eo 'inet (addr:)?([0-9]{1,3}\.){3}[0-9]{1,3}' | awk '{print $NF}')
  # 主节点启动
