...
def transfer_logs_as_json(log_file, output_json_file):
    ...
    log_pattern = re.compile(
        r"actor/entropy\s*:\s*([0-9.eE+-]+).*?"
        r"rollout_corr/log_ppl_diff\s*:\s*([0-9.eE+-]+).*?"
        r"actor/pg_loss\s*:\s*([0-9.eE+-]+).*?"
        r"actor/grad_norm\s*:\s*([0-9.eE+-]+)"
    )
    ...
    if log_matches:
        data["entropy"] = [float(match[0]) for match in log_matches]
        data["logp_diff"] = [float(match[1]) for match in log_matches]
        data["pg_loss"] = [float(match[2]) for match in log_matches]
        data["grad_norm"] = [float(match[3]) for match in log_matches]
